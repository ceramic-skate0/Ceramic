...
namespace Ceramic
{
    class DefenderCheck
    {
        //Code from https://github.com/matterpreter/DefenderCheck
            public static void DefenderCheckScan(string filepath)
            {
                Setup();
                bool debug = false;
            ...
            private static ScanResult Scan(string file, bool getsig = false)
            {
                if (!File.Exists(file))
                {
